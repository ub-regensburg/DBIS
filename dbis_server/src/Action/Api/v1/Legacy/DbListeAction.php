...
class DbListeAction extends ResourcesBaseAction
{
    ...
    public function __invoke(
        ...
        if ($ubr_id) {
            try {
                // getUbrIdFromQueryParams is very "tolerant", so we need to normalize $dbis_id:
                $dbis_id = $this->orgService->getDbisIdForUbrId($ubr_id);
            } catch (OrganizationWithDbisIdNotExistingException $e) {
                $response->getBody()->write('No organization with id ' . $ubr_id . ' found.');
        ...
        // 'excludes' => ['keywords.external_id', 'keywords.created_at', '*.resource_ids', 'subjects.is_top_database'],
        $search_client->enableSourceFiltering([
            'includes' => ['*'],
            'excludes' => [
                'keywords.id', 'keywords.external_id', 'keywords.created_at', 'keywords.modified_at', 'keywords.keyword_system',
                'alternative_titles.id', 'alternative_titles.external_id', 'alternative_titles.*_at', 'alternative_titles.*_date',
                'subjects.parent', 'subjects.subject_system', 'subjects.*_at',
                ],
        ]);
